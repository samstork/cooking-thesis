...
class RealAgent:
	...
	def get_status(self):
		own_subtask = self.subtask if self.subtask is not None else 'None'
		completeness = "Complete" if self.subtask_complete else "Incomplete"
		probs = self.delegator.get_probs()
		prob_string = "None"
	...
	def setup_subtasks(self, env):
		...
		self.delegator = BayesianDelegator(
				...
				bounding_factor=self.arglist.bounding_factor)
...
class SimAgent:
	...
	def get_status(self):
		return ("Sim Agent {} currently at {}, action {}, holding {}".format(
			color(self.name, self.color),
			self.location,
			self.action,
			self.get_holding()))
